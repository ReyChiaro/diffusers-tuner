...
class PairedCenterCrop:
    ...
    def __call__(
        ...
        target_h = int(target.shape[-2] / aspect_ratio)
        target_w = int(target.shape[-1] * aspect_ratio)
        i, j, h, w = get_centercrop_params(target.shape[-2:], [target_w, target_h])
        ...
        if isinstance(image, torch.Tensor):
            image = [image]
        for img in image:
            img_h = int(img.shape[-2] / aspect_ratio)
            img_w = int(img.shape[-1] / aspect_ratio)
            i, j, h, w = get_centercrop_params(img.shape[-2:], [img_w, img_h])
            input_images.append(
                T.resize(
                    T.resized_crop(img, i, j, h, w, [img_h, img_w]),
                    [target_h, target_w],
                )
            )
